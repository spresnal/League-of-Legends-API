...
Meteor.methods({
    ...
    getLeaguesByTeamIds: function (v) {
        this.unblock();
        return Meteor.http.call('GET', requestHelper.buildUrl(apiVersion, `/league/by-team/${v.teamIds}`));
    },
    ...
    getLeagueEntriesByTeamIds: function (v) {
        this.unblock();
        return Meteor.http.call('GET', requestHelper.buildUrl(apiVersion, `/league/by-team/${v.teamIds}/entry`));
    },
    ...
    getChallengerLeagues: function (v) {
        this.unblock();
        return Meteor.http.call('GET', requestHelper.buildUrl(apiVersion, `/league/challenger`));
    },
    // GET /api/lol/{region}/v2.5/league/master Get master tier leagues. (REST)
    // Return Value: LeagueDto
    getMasterLeagues: function (v) {
        this.unblock();
        return Meteor.http.call('GET', requestHelper.buildUrl(apiVersion, `/league/master`));
    }
});
